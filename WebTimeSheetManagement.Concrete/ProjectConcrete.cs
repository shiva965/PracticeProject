...
namespace WebTimeSheetManagement.Concrete
{
    public class ProjectConcrete : IProject
    {
        ...
        public List<ProjectMaster> GetListofProjects(int userid)
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["TimesheetDBEntities"].ToString()))
            {
                ...
                try
                {
                    ...
                    var result = con.Query<ProjectMaster>("GetProjectName", param, null, true, 0, System.Data.CommandType.StoredProcedure).ToList();
                    if (result.Count > 0)
                    {
                        return result;
                    }
