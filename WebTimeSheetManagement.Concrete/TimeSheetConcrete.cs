...
namespace WebTimeSheetManagement.Concrete
{
    public class TimeSheetConcrete : ITimeSheet
    {
        ...
        //    }
        //}
        public List<MainTimeSheetView>CheckIsDateAlreadyUsed(DateTime FromDate, int UserID)
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["TimesheetDBEntities"].ToString()))
            {
                ...
                try
                {
                    List<MainTimeSheetView> model = new List<MainTimeSheetView>();
                    using (var _context = new DatabaseContext())
                    {
                        var result = (from timesheetmaster in _context.TimeSheetMaster
                                      where timesheetmaster.FromDate == FromDate && timesheetmaster.UserID == UserID
                                      select timesheetmaster.TimeSheetMasterID);

                        int masterid = Convert.ToInt32(result.FirstOrDefault());
                        if (result != null && masterid > 0)
                        {
                            MainTimeSheetView objMT = new MainTimeSheetView();
                            objMT.ListTimeSheetDetails = TimesheetDetailsbyTimeSheetMasterID(UserID, masterid);
                            objMT.ListofProjectNames = GetProjectNamesbyTimeSheetMasterID(masterid);
                            objMT.ListofPeriods = GetPeriodsbyTimeSheetMasterID(masterid);
                            objMT.ListoDayofWeek = DayofWeek();
                            //objMT.description = GetDescriptionbyTimeSheetMasterIDandUserID(masterid, UserID);
                            objMT.TimeSheetMasterID = masterid;
                            model.Add(objMT);
                            //model = objMT;
                            // return model;
                        }
